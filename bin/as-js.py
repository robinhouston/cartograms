...
class AsJSON(object):
  ...
  def print_region_paths(self):
    ...
    for cart in self.carts:
      ...
      interpolators[cart_name] = utils.Interpolator(cart, self.m)
    
    empty_object_json = json.dumps( dict(( (k, {}) for k in interpolators.keys() )) )
    ...
    for region_name, p, breakpoints in self.region_paths():
      ...
      for k, interpolator in interpolators.items():
        if self.options.include_segments:
          self.segments = []
        ...
        print >>self.out, "{data_var}[{k}][{region_name}] = {path};".format(
          ...
          path=json.dumps(self.multipolygon_as_svg(p, interpolator, breakpoints)),
        )
  ...
  def _simplify(self, coords, breakpoints):
    ...
    for segment in self._segments(coords, breakpoints):
      ls = LineString(segment).simplify(tolerance=self.options.simplification, preserve_topology=False)
      for coord in ls.coords:
        if coord != prev:
          yield coord
        prev = coord
  
  def polygon_ring_as_svg(self, ring, interpolator, breakpoints):
    poly_arr = ["M"]
    first = True
    for x, y in self._simplify(ring.coords, breakpoints):
      if interpolator:
        x, y = interpolator(x, y)
  ...
  def multipolygon_as_svg(self, multipolygon, interpolator, breakpoints):
    path_arr = []
    for g in multipolygon.geoms:
      path_arr.append(self.polygon_ring_as_svg(g.exterior, interpolator, breakpoints))
      for interior in g.interiors:
        path_arr.append(self.polygon_ring_as_svg(interior, interpolator, breakpoints))
